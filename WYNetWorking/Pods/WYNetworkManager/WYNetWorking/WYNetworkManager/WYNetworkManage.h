...
#ifndef kIsNetwork
#define kIsNetwork     [WYNetworkHelper isNetwork]  // 一次性判断是否有网的宏
#endif

#ifndef kIsWWANNetwork
#define kIsWWANNetwork [WYNetworkHelper isWWANNetwork]  // 一次性判断是否为手机网络的宏
#endif

#ifndef kIsWiFiNetwork
#define kIsWiFiNetwork [WYNetworkHelper isWiFiNetwork]  // 一次性判断是否为WiFi网络的宏
#endif

typedef NS_ENUM(NSUInteger, WYNetworkStatusType) {
    /// 未知网络
...
typedef NS_ENUM(NSUInteger, WYRequestSerializer) {
    /// 设置请求数据为JSON格式
    WYRequestSerializerJSON,
    /// 设置请求数据为二进制格式
    WYRequestSerializerHTTP,
};

typedef NS_ENUM(NSUInteger, WYResponseSerializer) {
    /// 设置响应数据为JSON格式
    WYResponseSerializerJSON,
    /// 设置响应数据为二进制格式
    WYResponseSerializerHTTP,
};

/// 请求成功的Block
typedef void(^WYHttpRequestSuccess)(id responseObject);
...
typedef void (^WYHttWYrogress)(NSProgress *progress);

/// 网络状态的Block
typedef void(^WYNetworkStatus)(WYNetworkStatusType status);

@class AFHTTPSessionManager;
@interface WYNetworkManage : NSObject
