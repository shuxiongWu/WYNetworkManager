...
@implementation WYNetworkManage
...
+ (NSURLSessionTask *)GET:(NSString *)URL
    ...
    NSURLSessionTask *sessionTask = [_sessionManager GET:URL parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if (_isOpenLog) {WYLog(@"responseObject = %@",responseObject);}
        [[self allSessionTask] removeObject:task];
        ...
        if (_isOpenLog) {WYLog(@"error = %@",error);}
        [[self allSessionTask] removeObject:task];
...
+ (void)initialize {
    ...
    _sessionManager.responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"aWYlication/json", @"text/html", @"text/json", @"text/plain", @"text/javascript", @"text/xml", @"image/*", nil];
    // 打开状态栏的等待菊花
...
+ (void)setRequestSerializer:(WYRequestSerializer)requestSerializer {
    _sessionManager.requestSerializer = requestSerializer==WYRequestSerializerHTTP ? [AFHTTPRequestSerializer serializer] : [AFJSONRequestSerializer serializer];
}
    
+ (void)setResponseSerializer:(WYResponseSerializer)responseSerializer {
    _sessionManager.responseSerializer = responseSerializer==WYResponseSerializerHTTP ? [AFHTTPResponseSerializer serializer] : [AFJSONResponseSerializer serializer];
}
